...
spec =
  describe "execute" $ do
    it "is Left when robot moves out of bounds north" $ do
      let robot = Robot.new North (Position (1, 1))
      let planet = Planet.new (Position (1, 1)) robot
    ...
    it "is Left when robot moves out of bounds south" $ do
      let robot = Robot.new South (Position (0, 0))
      let planet = Planet.new (Position (1, 1)) robot
      execute Advance planet `shouldSatisfy` isLeft
    it "is Left when robot moves out of bounds west" $ do
      let robot = Robot.new West (Position (0, 0))
      let planet = Planet.new (Position (1, 1)) robot
      execute Advance planet `shouldSatisfy` isLeft
    it "progresses robot when in bounds" $ do
      let planet =
            Planet.new (Position (1, 1)) (Robot.new East (Position (0, 0)))
      let result = robot <$> execute TurnLeft planet
      result `shouldBe` Right (Robot.new North (Position (0, 0)))
